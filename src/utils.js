...
let utils = {
  ...
  ajax: type => (url, data, disabledErrorAlert) => new Promise((resolve, reject) => {
    ...
    fetch(url, p).then(res => {
      if (res.status === 200) {
        res.json().then(json => {
          if (json.data.status === 1006 && !res.url.includes('getUserInfo')) {
            // router.push('/login')
            console.log(router)
          } else {
  ...
  conversionTime: (stamp, dateTime) => {
    ...
    if (dateTime === 0) {
      ...
      return y + '年' + utils.conversionDate(m) + '月'
    } else if (dateTime === 8) {
      return y + '-' + utils.conversionDate(m) + '-' + utils.conversionDate(d)
    }
  },
  move: (e) => {
    e.preventDefault()
  ...
  secondToTime: (sec) => {
    ...
    sec = sec - min * 60
    return utils.secondTo(min) + ':' + utils.secondTo(sec)
  },
  secondTo: (val) => {
    val = Math.floor(val)
    if (val < 10) {
